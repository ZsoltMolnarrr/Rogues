...
/// items (`ItemTags.STONE_TOOL_MATERIALS`, ...) it is used directly instead of a tag defined here.
///
/// Weapons pass `null` when the repair material is the tier's own `ToolMaterial` material — that keeps
/// the vanilla tag (iron ingot for T1, diamond for T2, netherite ingot for T3, gold ingot for GOLDEN).
///
/// Contents are emitted by `RoguesDataGenerator.ItemTagGenerator`.
public class RogueItemTags {
    ...
    private static TagKey<Item> repairs(String material, List<Identifier> required, List<Identifier> optional) {
        var tag = TagKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(RoguesMod.NAMESPACE, "repairs_" + material));
